...
PST_OCI_STAT_REGISTRY ?= $(CAR_OCI_REGISTRY_HOST)
PST_OCI_SEND_REGISTRY ?= $(CAR_OCI_REGISTRY_HOST)

# builder image always has to be from GitLab as its not pushed to CAR befuile runtime is built, so in a release build
PST_OCI_SEND_BUILDER ?= $(PST_OCI_SEND_REGISTRY)/$(PST_OCI_SEND_BUILDER_IMAGE):$(PST_OCI_SEND_BUILDER_TAG)
